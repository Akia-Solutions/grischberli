...
class BM_Product_Meta {
	...
	protected function get_rrp_meta( $product_id ) {
		...
		<div class="group-price-container">
			...
			<input class="space-right" type="number" step="0.0001" min="0" name="bm_rrp" value="<?php echo esc_attr( $rrp ); ?>" id="bm_rrp"><br>
		</div>
		<?php
	}
			...
			<div class="beefup__body">
						...
						if ( isset( $group_price ) && ! empty( $group_price ) ) {
							foreach ( $group_price as $price ) {
								...
								<?php if ( ! empty( $price['group_price'] ) ) : ?>
									<p>
										<label for="<?php echo esc_attr( $group_slug ); ?>group_price[<?php echo esc_attr( $counter ); ?>][group_price]"><?php esc_html_e( 'Price', 'b2b-market' ); ?></label>
				...
				<script>
					...
					$group_prices(document).ready(function() {
						...
						$group_prices(document).on('click', '.remove', function(){
			...
			<div class="beefup__body">
				...
				<script>
					...
					$bulk_prices(document).ready(function() {
						...
						$bulk_prices(document).on('click', '.remove', function(){
	...
	public function save_meta( $post_id ) {
		...
		foreach ( $current_groups as $group_id ) {
			...
			/* save copy options */
			$group_copy = $_POST[ 'bm_' . $group_slug . 'copy_for_group' ];
			...
			$bulk_copy = $_POST[ 'bm_' . $group_slug . 'bulk_copy_for_group' ];
