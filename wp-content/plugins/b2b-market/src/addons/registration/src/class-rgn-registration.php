...
class RGN_Registration {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_thankyou', array( $this, 'add_vat_to_order' ), 10, 1 );
	}
	...
	/**
	 * Update order vat id with B2B vat id.
	 *
	 * @param  int $order_id current order id.
	 * @return void
	 */
	public function add_vat_to_order( $order_id ) {
		...
		// Check if there is no vat added yet.
		$vat_id = $order->get_meta( 'billing_vat' );

		if ( empty( $vat_id ) ) {
			// Update order meta.
			update_post_meta( $order_id, 'billing_vat', $b2b_vat_id );
		}
	}
}
