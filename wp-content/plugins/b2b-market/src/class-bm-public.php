...
class BM_Public {
	...
	public function add_frontend_assets() {
		...
		wp_enqueue_script( 'bm-admin-bar', B2B_PLUGIN_URL . '/assets/admin/bm-admin-bar.' . $min . 'js', array( 'jquery' ), '1.0.8.2', true );
		wp_localize_script( 'bm-admin-bar', 'ajax', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );

		if ( $message_option ) {
			wp_enqueue_script( 'bm-discount-banner', B2B_PLUGIN_URL . '/assets/public/bm-discount-banner.' . $min . 'js', array( 'jquery' ), '1.0.8.2', true );
		}
	}
}
